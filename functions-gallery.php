...
class Functions{
	...
	sparql_ns( "gallery","http://www.semanticweb.org/anniedumashie/ontologies/2021/10/art-gallery#" ); // have to change this namespace to your ontology
    }
    ...
    public function get_nationalities(){
        ...
        $sparql = "SELECT ?nationality WHERE {
            ?x a gallery:Nationality
            bind(strafter(str(?x),str(gallery:)) as ?nationality)
          }";
    ...
    public function get_featured_artists(){
        $sparql = "SELECT ?name ?nationality WHERE {
            ?x a gallery:Artist;
                  gallery:name ?name;
